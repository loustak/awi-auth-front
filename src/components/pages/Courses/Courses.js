...
const courses = [
  ...
  { name: 'Cours 2', teacher:'Arnaud Castelltort', hours:'33', coef:'1' , intervenants:['Intervenant 1','Intervenant 2'] },
          ...
          }
          {
            courses.filter(course => course.name.toLowerCase().includes(search)).map((course, i) => // TODO
              <> <Collapse title={course.name} subtitle={course.teacher ? course.teacher : ''} key={i}>
                  <CourseItem hours={course.hours} description={course.description} coef={course.coef} intervenants={course.intervenants} key={i} />
                </Collapse>
                <br />
              </>
            )
